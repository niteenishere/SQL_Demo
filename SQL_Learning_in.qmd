...
select mpg, hp, gear, cyl from mtcars order by mpg desc; 
```
...
select Species , `Sepal.Length`, `Petal.Length` from iris order by Species ASC ,`Petal.Length` DESC;
...
select * from iris where Species = "versicolor"or species="setosa" and 'Sepal.Length' > 5.9;
...
select * from mtcars where cyl = 6 and hp < 120 order by mpg desc;
...
select count(Species) as versicolor_setosa_species from iris where Species = 'versicolor' or species = 'setosa';
...
select avg(hp) as average_horsepower_with_hp_more_than_150 from mtcars where hp > 150 and gear > 4;
...
select Species, variance('Sepal.Width') as var_Sepal_Length , count(*) as num_of_flowers from iris group by Species; 
...
select T1.name, T1.email, T2.product, T2.amount from Customer as T1 Inner join Orders as T2 on T1.customer_id = T2.customer_id;
...
select C.name , O.amount from Customer as C Left join Orders as O on C.customer_id = O.customer_id where amount is null;
...
update Customer set email=case customer_id when 101 then "Alice.2001@gmail.com" when 102 then "Bob.2002@gmail.com" else email -- this keep all other emails remain unchanged
